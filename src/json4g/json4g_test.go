...
func TestNewNode(t *testing.T) {
	...
	fmt.Println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
	var pm25 map[string]string = make(map[string]string)
	var voc map[string]string = make(map[string]string)
	...
	var level [3]map[string]string
	level[0] = pm25
	level[1] = voc
	a := new(JsonNode)
	b := NowJsonNode("b", "sdfsdf")
	c := NowJsonNode("c", "vasdfsw33333lue")
	//a.AddNode(b)
	//a.AddNode(c)

	a.ArraysStruct = make([]Inode, 3)
	a.ArraysStruct[0] = b
	a.ArraysStruct[1] = c
	a.ArraysStruct[2] = b
	fmt.Println("newnode>>>>>>")
	...
	a.Name = "level"
	root := new(JsonNode)
	root.setJsonValue(pm25)
	root.Name = "root"
	fmt.Println("newnode>>>>>>", a.ToCouchDBString())
	fmt.Println("newnode>>>>>>", root.NType)
