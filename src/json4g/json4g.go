...
func (n *JsonNode) ToCouchDBString() (s string) {
	defer func() {
		if er := recover(); er != nil {
			fmt.Println("ToString error ", er)
		}
	}()
	return _toCouchDBString(n)
}

func _toCouchDBString(n *JsonNode) (s string) {
	...
	case STRUCT:
		...
		for _, dat := range dats {
			s = fmt.Sprint(s, _toCouchDBString(dat.ToJsonNode()))
			if i < len(dats)-1 {
				s = fmt.Sprint(s, ",")
			}
			i++
